...
static void _sample(struct mTiming* timing, void* user, uint32_t cyclesLate) {
	...
	if (_kirbyTraceMgbaAudio(audio)) {
		fprintf(stderr,
		        ...
		        audio->psg.ch2Left ? 1 : 0,
		        audio->psg.ch3Left ? 1 : 0,
		        audio->psg.ch4Left ? 1 : 0,
		        audio->psg.ch1Right ? 1 : 0,
		        audio->psg.ch2Right ? 1 : 0,
		        audio->psg.ch3Right ? 1 : 0,
		        audio->psg.ch4Right ? 1 : 0,
		        audio->psg.playingCh1 ? 1 : 0,
		        audio->psg.playingCh2 ? 1 : 0,
		        audio->psg.playingCh3 ? 1 : 0,
		        audio->psg.playingCh4 ? 1 : 0,
		        audio->psg.ch1.sample,
